...
export function IssueDetail() {
  ...
  const sourceBreadcrumb = useMemo(
    () => readIssueDetailBreadcrumb(location.state, location.search) ?? { label: t("issues"), href: "/issues" },
        ...
        title:
          variables.sharingPreferenceAtSubmit === "prompt"
            ? variables.allowSharing
              ? t("feedbackSavedShareFuture")
              : t("feedbackSavedLocalFuture")
            : variables.allowSharing
              ? t("feedbackSavedSharingEnabled")
              : t("feedbackSaved"),
        tone: "success",
      });
    },
  ...
  const archiveFromInbox = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("unableToArchiveFromInbox"),
        tone: "error",
      });
    },
  });
        ...
        >
          <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
            <span className="text-sm font-medium text-muted-foreground">
              {t("linkedApprovalsCount", { count: linkedApprovals.length })}
            </span>
            <ChevronDown
