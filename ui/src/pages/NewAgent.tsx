...
export function NewAgent() {
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("failedToCreateAgent"));
    },
  });
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="border border-border">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("thisWillBeCeo")}</p>
          )}
          {formError && (
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            >
              {createAgent.isPending ? t("creating") : t("createAgent")}
            </Button>
          </div>
        </div>
      </div>
