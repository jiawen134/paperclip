...
export function CliAuthPage() {
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">
            {challenge.status === "expired" ? t("cliAuthChallengeExpired") : t("cliAuthChallengeCancelled")}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("restartCliAuthFlow")}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("approvePaperclipCliAccess")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
        ...
        <div className="mt-5 space-y-3 text-sm">
          ...
          <div>
            <div className="text-muted-foreground">{t("requestedAccess")}</div>
          ...
          {challenge.requestedCompanyName && (
            <div>
              <div className="text-muted-foreground">{t("requestedCompany")}</div>
        ...
        {(approveMutation.error || cancelMutation.error) && (
          <p className="mt-4 text-sm text-destructive">
            {(approveMutation.error ?? cancelMutation.error) instanceof Error
              ...
              : t("failedToUpdateCliAuthChallenge")}
          </p>
        )}

        {!challenge.canApprove && (
          <p className="mt-4 text-sm text-destructive">
            {t("requiresInstanceAdminAccess")}
          </p>
        )}
        ...
        <div className="mt-5 flex gap-3">
          ...
          >
            {approveMutation.isPending ? t("approving") : t("approveCliAccess")}
          </Button>
          <Button
            type="button"
            variant="outline"
